...
class TextProcessing(object):
    def __init__(self, cmudict, symbol_set, cleaner_names, p_arpabet=0.0,
    ...
    def get_arpabet(self, word, print_text=False):
        arpabet_suffix = ''

        if word.lower() in self.cmudict.heteronyms:
            if print_text:
                print("=== returning get_arpabet")
            return word
        ...
        if print_text:
            print(f'get_arpabet: arpabet, {arpabet}')
    ...
    def encode_text(self, text, return_all=False, use_arpabet=False, print_text=False):
        ...
        if self.p_arpabet > 0:
            if self.handle_arpabet == 'sentence':
                ...
                if print_text:
                    print(f'words, {words}')
                text_arpabet = [
                    # word[1] if word[0] == '' else (self.get_arpabet(word[0], print_text=print_text) if use_arpabet < self.p_arpabet else word[0])
                    word[1] if word[0] == '' else (self.get_arpabet(word[0], print_text=print_text) if use_arpabet else word[0])
                    for word in words]
                if print_text:
                    print(f'text_arpabet, {text_arpabet}')
                text_arpabet = ''.join(text_arpabet)
                text = text_arpabet
                ...
                raise Exception("{} handle_arpabet is not supported".format(
                    self.handle_arpabet))
            if print_text:
                print("ARPAbet text", text)
                print("use_arpabet", use_arpabet)
                print("self.handle_arpabet", self.handle_arpabet)
        else:
            if print_text:
