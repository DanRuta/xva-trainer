...
try:
    ...
    from util import piecewise_rational_quadratic_transform
...
class StochasticDurationPredictor(nn.Module):
    ...
    def forward(self, x, x_mask, dr=None, g=None, lang_emb=None, lang_emb_full=None, reverse=False, noise_scale=1.0):
        """
        Shapes:
            - x: :math:`[B, C, T]`
            - x_mask: :math:`[B, 1, T]`
        ...
        if lang_emb is not None:
            if lang_emb_full is not None:
                x = x + self.cond_lang(lang_emb_full)
            else:
