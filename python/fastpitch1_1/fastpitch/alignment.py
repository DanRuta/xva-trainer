...
def mas_width1_gpu(attn_map):
    ...
    for i in range(attn_map.shape[0]-1, -1, -1):
        opt[i, curr_text_idx] = 1
        curr_text_idx = prev_ind[i, curr_text_idx].long()
    opt[0, curr_text_idx] = 1
    return opt
def b_mas_gpu(b_attn_map, in_lens, out_lens, width=1):
    attn_out = torch.zeros_like(b_attn_map)
    for b in prange(b_attn_map.shape[0]):
        out = mas_width1_gpu(b_attn_map[b, 0, :out_lens[b], :in_lens[b]])
